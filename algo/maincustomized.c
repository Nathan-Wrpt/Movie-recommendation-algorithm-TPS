...
int main(){
    ...
    updateGraph(graph, users, NBUSERS - 5, clientsParsed, numClients, privilegedClientsParsed, numPrivilegedClients, minmoviesreviewed, dateLimit, weights);
    ...
    for(int i = 0; i < numMoviesRecommended; i++){
        printf("-%s\n", movies[closestMoviesID[i] - 1].title);
    }
    ...
    free(clients);
    free(clientsParsed);
    free(privilegedclients);
    free(privilegedClientsParsed);
    free(response);
    free(folderpath);
